...
function displayPrivacyDetails() {
    ...
    let privacyHeader = `<h2>Privacy Details</h2>`;
    detailsDiv.innerHTML = privacyHeader;
    ...
    detailsDiv.innerHTML += `
        <p><strong>Cookies:</strong> ${cookiesEnabled}</p>
        <p><strong>Local Storage:</strong> ${localStorageEnabled}</p>
    `;

    // Navigator Storage Quota to detect Incognito Mode
    navigator.storage.estimate().then(estimate => {
        let quotaInfo = `
            <p><strong>Storage Quota:</strong> ${(estimate.quota / (1024 * 1024)).toFixed(2)} MB</p>
            <p><strong>Used Storage:</strong> ${(estimate.usage / (1024 * 1024)).toFixed(2)} MB</p>
        `;
